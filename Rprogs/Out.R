...
parser <- ArgumentParser(description="Find and modify outliers")
parser$add_argument("-v", "--verbose", action="store_true", default=FALSE,
                    help="verbose help [default %(default)s]")
parser$add_argument("-i","--ifile", default="data/TCE1.csv",
                    help="File with _numeric_ values default %(default)s]")
parser$add_argument("--find", action="store_true", default=FALSE,
                    help="Find outliers and sort by most outside the IQR [default %(default)s]")
parser$add_argument("--squish", default="",
                    help="Squish cols in list. I.e replace out with 5 or 95 percentile [default %(default)s]")
parser$add_argument("--remove0", action="store_true", default=FALSE,
                    help="Replace 0 with NA [default %(default)s]")
...
#parser$add_argument("--omit", default="koi_disposition,kepid",
#                    help="List of cols to omit n.b. if koi_disposition is not omitted it will be made numeric) [default %(default)s]")
...
if(!grepl("kepid", argv$omit, fixed=TRUE)){
    print(paste0("kepid must be among values omitted"))
    quit()
}
